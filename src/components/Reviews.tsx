      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="text-center max-w-xl mx-auto">
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            ...
            <span className="font-medium text-brand-navy">
              {avaliacoesResumo.nota.toFixed(1).replace(".", ",")}
            </span>
        ...
        <div className="mt-12 md:mt-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {avaliacoes.map((avaliacao, i) => (
            <Reveal key={avaliacao.id} delay={i * 90} className="flex">
              <blockquote className="relative rounded-2xl bg-white border border-brand-navy/10 p-6 flex flex-col flex-1">
                <Quote
                ...
                <footer className="mt-5 flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-0.5" aria-hidden="true">
                    {Array.from({ length: avaliacao.nota }).map((_, estrela) => (
                      <Star
                        key={estrela}
